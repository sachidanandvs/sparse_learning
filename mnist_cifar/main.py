...
def evaluate_truncated_data(args, model, device, test_loader, is_test_set=True):
    model.eval()
    ...
    for truc in trucation:
        ...
        print_and_log('{}: For test data with truncation {}-{}% Average loss: {:.4f}, Accuracy: {}/{} ({:.3f}%)\n'.format(
            'Test evaluation' if is_test_set else 'Evaluation', truc[0], truc[1],
            test_loss, correct, n, 100. * correct / float(n)))
...
def main():
    ...
    for i in range(args.iters):
        ...
        evaluate_truncated_data(args, model, device, test_loader, is_test_set=True)
        print_and_log("\nIteration end: {0}/{1}\n".format(i+1, args.iters))
